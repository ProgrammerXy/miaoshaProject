...
public class UserController extends BaseController{
    ...
    public CommonReturnType login(@RequestParam(name = "telphone")String telphone,@RequestParam(name = "password")String password) throws BusinessException, UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        //用户登录服务,校验用户登录是否合法
        UserModel userModel = userService.vailDataLogin(telphone,this.EncoderMd5(password));
        //加入到用户登录成功的session内
    ...
    public CommonReturnType register(@RequestParam(name = "telphone")String telphone,
        ...
        if(!com.alibaba.druid.util.StringUtils.equals(otpCode,inSessionOtpCode)){
            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"otp验证不一致");
        }
        //用户的注册流程
        ...
        userModel.setEncrptPassword(this.EncoderMd5(password));
    ...
    public CommonReturnType getOtp(@RequestParam(name = "telphone")String telphone){
        ...
        //讲OTP验证码同对应用户的手机号关联,使用HttpSession的方式绑定手机号与OtpCode
        httpServletRequest.getSession().setAttribute(telphone,otpCode);
        //讲OTP验证码通过短信通道发送给用户,省略
